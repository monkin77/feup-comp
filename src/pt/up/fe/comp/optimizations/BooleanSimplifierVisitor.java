...
public class BooleanSimplifierVisitor extends PostorderJmmVisitor<Object, Boolean> {
    ...
    private Boolean andExprVisit(JmmNode node, Object o) {
        ...
        if (!lhsLhs.getKind().equals("_Identifier")) return false;
        JmmNode lhsRhs = lhs.getJmmChild(1);
        if (!lhsRhs.getKind().equals("_Identifier")) return false;
        JmmNode rhsLhs = rhs.getJmmChild(0);
        if (!rhsLhs.getKind().equals("_Identifier")) return false;
        JmmNode rhsRhs = rhs.getJmmChild(1);
        if (!rhsRhs.getKind().equals("_Identifier")) return false;
        String lhsLhsId = lhsLhs.get("id");
        String lhsRhsId = lhsRhs.get("id");
        String rhsLhsId = rhsLhs.get("id");
        String rhsRhsId = rhsRhs.get("id");
